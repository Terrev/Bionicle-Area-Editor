...
public class Manager : MonoBehaviour
{
	...
	public void LoadObjSlb()
	{
		...
		string path = Application.dataPath + "/Resources/" + levelName + "/" + areaName + "/" + areaName + "_OBJ.slb";
		...
		for (int i = 0; i < entryCount; i++)
		{
			...
			GameObject newGameObject = Instantiate(Resources.Load(levelName + "/" + areaName + "/" + identifier, typeof(GameObject)), location, Quaternion.Euler(orientation), slbParent.transform) as GameObject;
	...
	public void SaveObjSlb()
	{
		...
		// grab the area's objects/file entries
	...
	{
		// PREPARE FOR FILE READING
		string path = Application.dataPath + "/Resources/" + levelName + "/" + areaName + "/" + areaName + "_POS.slb";
		FileStream fileStream = new FileStream(path, FileMode.Open);
		BinaryReader binaryReader = new BinaryReader(fileStream);
